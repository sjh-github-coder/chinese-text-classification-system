...
agent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36'
headers = {'User-Agent': "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36"}
...
def get_comment(newurl):
    max_id = ""
    cookie_path = ""
    # url = ""
    print('finish weibo_comment step1')
    while True:
        if max_id == "":
            url = newurl
            url = "https://m.weibo.cn/comments/hotflow?id=4445488834869489&mid=4445488834869489&max_id_type=0"  # 评论首页地址
        else:
            # print(url[:-7])  # 截取从头开始到倒数第7个字符之前
            url = url[:-7] + "=" + str(max_id) + "&max_id_type=0"
            # url = "https://m.weibo.cn/comments/hotflow?id=4176281144304232&mid=4176281144304232&max_id=" + str(  # 评论第二页及以后的地址
            # max_id) + "&max_id_type=0"
            break
        # print(url)
        response = requests.get(url, headers=headers, cookies=cookie_path)
        ...
        print('finish comment ok')
        # print([data_1["text"] for data_1 in comment["data"]["data"]])
        line = []
        a = open(r"D:\\python\\PycharmProjects\\Emo_classify\\text_emo\\static\\classify\\data\\wb_comment.txt", "w", encoding='utf-8')
...
weibo.login()
weibo_comment()
get_comment("https://m.weibo.cn/comments/hotflow?id=4445488834869489&mid=4445488834869489&max_id_type=0")
print('finish get_cha')
print('finish wb_comment')
