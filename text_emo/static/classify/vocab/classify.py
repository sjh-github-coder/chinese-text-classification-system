...
def seg_word(sentence):
    ...
    # 读取停用词文件
    stopwords = set()
    fr = codecs.open('D:/python/PycharmProjects/Emo_classify/text_emo/static/classify/vocab/data/stopwords.txt', 'r', 'utf-8')
    for word in fr:
...
def classify_words(word_dict):
    """词语分类,找出情感词、否定词、程度副词"""
    # 读取情感字典文件
    sen_file = open('D:/python/PycharmProjects/Emo_classify/text_emo/static/classify/vocab/data/BosonNLP_sentiment_score.txt', 'r+', encoding='utf-8')
    # 获取字典文件内容
    ...
    # 读取否定词文件
    not_word_file = open('D:/python/PycharmProjects/Emo_classify/text_emo/static/classify/vocab/data/notDic.txt', 'r+', encoding='utf-8')
    # 由于否定词只有词，没有分值，使用list即可
    not_word_list = not_word_file.readlines()

    # 读取程度副词文件
    degree_file = open('D:/python/PycharmProjects/Emo_classify/text_emo/static/classify/vocab/data/degree.txt', 'r+', encoding='utf-8')
...
# 测试
a=[]
with codecs.open('D:/python/PycharmProjects/Emo_classify/text_emo/static/classify/vocab/data/test.txt','r',encoding='utf-8') as f:
    sample=f.readlines()
    for line in sample:
        score = setiment_score(line)
        if score > 0.8:
            score="pos"
        elif score < -0.8:
            score="neg"
